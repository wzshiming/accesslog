...
import (
	...
	"github.com/wzshiming/accesslog/oss"
...
var (
	...
	fields    = oss.AccessLogFormatted{}.Fields()
	condition string
)
...
func run(
	...
	cacheHTTPClient := oss.NewCacheHTTPClient(nil, cache)

	client, err := oss.NewOSSClient(
		endpoint,
		accessKeyID,
		accessKeySecret,
		oss.HTTPClient(cacheHTTPClient),
	)
	if err != nil {
		return err
	}

	ch := make(chan oss.AccessLogFormatted, 128)
	...
	return tocsv.ProcessToCSV[oss.AccessLogFormatted](os.Stdout, condition, fields, ch)
}
