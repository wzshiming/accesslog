...
import (
	...
	"github.com/wzshiming/accesslog/obs"
...
var (
	...
	fields    = obs.AccessLogFormatted{}.Fields()
	condition string
)
...
func run(
	...
	cacheHTTPClient := obs.NewCacheHTTPClient(nil, cache)

	client, err := obs.NewOBSClient(
		accessKeyID,
		accessKeySecret,
		endpoint,
		obs.WithHttpClient(cacheHTTPClient),
	)
	if err != nil {
		return err
	}

	ch := make(chan obs.AccessLogFormatted, 128)

	go func() {
		defer close(ch)
		for i := start; i.Before(end); i = i.Add(time.Hour) {
			date := i.UTC().Format(dataFormat) + "-"
			err := obs.ProcessAccessLogWithClient(client, bucketName, date, func(entry accesslog.Entry[obs.AccessLog], err error) error {
				if err != nil {
					return err
				}
	...
	return tocsv.ProcessToCSV[obs.AccessLogFormatted](os.Stdout, condition, fields, ch)
}
